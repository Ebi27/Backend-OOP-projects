...
import default  from "./modules/salaryData.js";
import {
  getRoles,
  getCompanies,
  getDataByRole,
  getDataByCompany,
} from "./modules/salaryData";
import {
  getAverageSalaryByRole,
  getAverageSalaryByCompany,
  getAverageSalaryAtCompany,
  getIndustryAverageSalary,
} from "./modules/workAroundModule";

const {salaryData} = domFunctions;
...
export function updateResults() {
  ...
  const averageSalaryByRole = getAverageSalaryByRole(roles);
  const averageSalaryByCompany = getAverageSalaryByCompany(companies);
  const salary = getAverageSalaryAtCompany(roles, companies);
  const industryAverageSalary = getIndustryAverage();
